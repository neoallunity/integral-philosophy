...
% Луговская Е.Г. - русская статья
    ...
    Настоящее исследование направлено на применение аппарата Проективно-модальной онтологии (ПМО) В.И. Моисеева (Моисеев, 2002, 2004) к анализу образа Соловьева у МережковскогоПМО рассматривается здесь не как универсальный метод анализа художественного текста, но как метаязык описания для концептуальных структур, выявленных лингвориторическим анализом Допускается метатеоретическое расширение: введение понятия множества собственных моделей для описания условий, при которых модус (личность) способен образовывать свои моды (аспекты существования).
     формализовать концептуальную структуру образа В.С. Соловьева посредством предикатного синтаксиса ПМО, выявив онтологическую архитектонику трагедии «немого пророка» и верифицировав результаты против установленных лингвориторическим анализом фактов.
    Теоретические основания: Проективно-модальная онтология В.И. Моисеева
    2.1. Базовый аппарат ПМО
    Проективно-модальная онтология В.И. Моисеева разработана на основе онтологии С. Лесьневского с использованием категориального синтаксиса (Моисеев, 2002). Центральное место в ПМО занимает , имеющий следующий категориальный тип:
    
    [width=1]

    где [width=1]– тип предложений, [width=1]– тип имен, [width=1]– произвольный категориальный тип, [width=1]– тип двуместных функторов. 
 ...
 &  &  & 
    ...
    Вводим понятие множества собственных моделей [width=1]как совокупности условий [width=1], на которых модус [width=1]способен образовывать свои моды. Формально: 
    
    [width=1]

    То есть модель [width=1] входит в множество собственных моделей модуса [width=1], если существует проектор [width=1], образующий из [width=1]в условии [width=1]некоторую моду [width=1]. 
    Обратно, если модель [width=1]не входит в множество собственных моделей модуса:
    
    [width=1]

    то есть не существует проектора который смог бы произвести моду из модуса [width=1]в модели [width=1]. Это – онтологическая невозможность а не выбор или негативное суждение.

    ПМО используется здесь не как самостоятельный метод анализа литературно-критического текста Д. С. Мережковскогоно какметаязык для описания и формализации концептуальных структур, выявленных лингвориторическим анализом.
    ...
    Предлагаем следующую схему использования конструкций ПМО для указанных целей. 
       ...
       & Лингвориторический анализ АСП, риторические приемы & Выявление концептов «философ-созерцатель» / «человек-деятель», символических деталей, семантического напряжения \ 
      
       & ПМО-синтаксис (Mod-предикаты) & Формализация концептов как модусов и мод, определение моделей и условий \ 
      
       & Анализ множеств собственных моделей, валентные определения & Выявление точек трагедии, онтологических апорий, невозможных мод \ 
      
       & Сопоставление результатов ПМО и Л-анализа & Подтверждение или коррекция интерпретаций \ 
      
    \

    
    Представим базовый модус: 
    ...
    Обозначим этот модус как [width=1]для краткости. Модус [width=1]обладает следующими сущностными структурными предикатами, которые принадлежат самому модусу, а не образуются как моды на различных моделях:
    
       [width=1]: Приоритет разума (гнозис) над волей 
       [width=1]: «Богоделание вытекает из богопознания» 
       [width=1]: Рационалистичность 
       [width=1]: Консервативность и ретроградность 
       [width=1]: Реставраторство («Да будет снова то, что было») 
    

    Определение моды модуса Y в контексте $$:
    
    [width=1]

    То есть [width=1] есть мода модуса [width=1]в некотором контексте [width=1], если существуют условие (модель) [width=1], проектор [width=1], которые образуют [width=1] из [width=1]. 
    На этом этапе моделирование необходимо сделать отдельное замечание о связи ПМО-описания с ассоциативно-семантическими полями (АСП)}: 
    В лингвориторическом анализе (Луговская, Грудина, 2024) выявлены два ассоциативно-семантических поля:
    
       АСП «философ-созерцатель» (созерцание, погруженность в идеальное, подполье, тайна)
       АСП «человек-деятель» (реформатор, деятельность, видимая сторона)
    
    В ПМО-описании эти семантические поля переструктурируются как:
    
       АСП соответствует множеству вторичных предикатов, которые маркируют доступ к моде
       Каждый концепт в АСП указывает на условие (модель), при котором образуется мода
       Модус [width=1]рассматривается как единственный источник, из которого проектируются различные моды 
    ...
    Модус не образует моды на всех мыслимых условиях. Множество собственных моделей [width=1]модуса [width=1]– это те условия (модели), на которых модус способен образовывать свои моды. 
    Базовая формула:
    ...
    Модель 1: Прошлое [width=1]и мода реставратора определяется следующим условием модели Рассмотрение [width=1]в условиях прошлого, истории, традиции. 
    ...
    Читается: «[width=1] есть мода модуса [width=1](гностика-созерцателя) в модели прошлого [width=1]с проектором [width=1]». 
    Характеристики моды:
    
       Содержание: романтик былого, тайный славянофил, реставратор
       Поле образования: отношение к историческому времени
       Символический репрезентант: розовый башмачок
    

    Рассмотрим символический репрезентант розовый башмачок как материализованную модель и точку амбивалентности, для чего дадим некоторое уточнение к установке лингвориторического анализа}: 
    Розовый башмачок функционирует в критическом тексте Мережковского двойственно:
    Во-первых, как материализованная модель
    [width=1]– конкретное физическое условие, объект, через который проектируется мода реставратора. Башмачок – это материальный знак идеального чувства(платонической любви к Софье Хитрово), что создает амбивалентностьвещь одновременно указывает на прошлое (его материальность, эмпирическое существование) и на идеальное (неосуществимое чувство, невозвратимость).
    ...
    Пусть башмачок обозначен как объект [width=1]. Тогда: 
    
    [width=1]

    означает, что мода [width=1]образуется из модуса [width=1]в условии, где модель сама является материальным объектом [width=1], одновременно являющимся: 
    
       Репрезентантом прошлого [width=1]
       Точкой амбивалентного перехода (граница между чистым созерцанием и деятельностью)
    ...
    Рассмотрим следующую модель: Настоящее [width=1]и мода консерватора 
    Условием этой модели в структуре образа В. С. Соловьева у Д. С. Мережковского является рассмотрение [width=1]в условиях современности, необходимости сохранения существующего. 
    Текстовые маркеры:
    ...
    Обозначим характеристики этой моды:
       ...
       Содержание: консервирующая деятельность, поддержание существующего
       Поле образования: отношение к историческому процессу
       Риторический маркер: консервативность, ретрограднось
    

    В локиге нашего анализа следующей к рассмотрению выступает модель: Будущее как эсхатон [width=1]и мода эсхатолога 
    Ее условием выступает рассмотрение [width=1]в условиях апокалиптического будущего, конца истории. 
    Текстовые маркеры:
    ...
    Важно отметить, что по Д. С. Мережковскому будущее для В. С. Соловьева – не пространство созиданиянокатастрофа, апокалипсис Модель [width=1]принципиально отлична от революционной, прагматической модели (см. раздел 4.3). Эсхатологическое будущее – это конец истории, а не ее продолжение и преобразование. 
    То есть характеристиками этой моды будут следующие:
    
       Содержание: апокалиптический пророк, предсказатель антихриста
       Поле образования: отношение к будущему времени (как апокалипсису, а не историческому прогрессу)
    ...
    Как видим, темпоральная структура образа В. С. Соловьева у Д. С. Мережковского состоит втрехмерном варьировании одного и того же модусана условиях трех темпоральных моделей. Каждая мода полнокровна, каждая содержит истину о Соловьеве, но 

    

    В дальнейшей логике построения модели образа В. С. Соловьева в литературно-критическом тексте Д. С. Мережковского мы приходим к необходимости определения невозможной модели, ключевой тезой которой выступает утверждение, что 	extit	extit
    [width=1]
    ...
    Другими словами, это не выбор и не оценкано	extit Модус [width=1]структурно несовместим с условием [width=1]. 
    Текстовые маркеры:
    ...
    Оппозиция указывает на качественное несоответствиемежду интенсивностью гностического созерцания и требуемой интенсивностью исторического действия.
    Метафора
    Критическая мощь В. С. Соловьева (жало) несоразмерна твердости институционального сознания (кожа гиппопотама). Глагол «не почесалось»маркирует полное отсутствие эффекта, что говорит не о слабости жала, но о его неприложимости к этому материалу.
    Метафора 
    ...
    где [width=1]– модель скрытого бытия (недоступного внешнему наблюдению), [width=1]– модель публичного дискурса (дискурсивно выраженного). 
    ...
    ПМО-характеристики:
       ...
       Онтологически истинна (содержит подлинную профетическую истину)
       Эпистемологически недоступна (невыразима в публичном дискурсе)
       Семиотически немая (не артикулируется в философских трудах)
    
    Замечание о связи с АСП}: 
    В лингвориторическом анализе концепт скрытости реализуется через следующие лексемы и образы:
    
       скрывается за произведениями
       стальная решетка на лице
       подполье и чертовщина
       невидимая сторона медали
       то, что не показывают публично
    
    Эти лексемы маркируют условие невыраженности, условие [width=1], в котором образуется мода скрытого пророка. 
    
    Текстовые маркеры:
    «В произведениях все стройно, ясно, гладко, даже слишком гладко, выглажено, вылощено» [Там же: 129, 118].
    Серия синонимичных адъективов с семой  и ( (упорядоченность),  (отчетливость),  (отсутствие шероховатости),  (механическая обработка),  (искусственная полировка) и семантическое накопление создает эффект избыточности. В синонимическом ряду возникает эффект катахрезы – столкновение положительной оценки (ясность, стройность) и негативной коннотации (искусственность, маска) в рамках одной семы . 
    ПМО - характеристики:
    
       Дискурсивно выражена («десять томов философии»)
       Онтологически ложна (маска, публичный образ)
       Семиотически избыточна («слишком гладко, вылощено»)
    ...
    Профетическая истина Соловьева  в модели публичного дискурса [width=1], а выраженное в публичном дискурсе (философия в десяти томах) не содержит профетической истины.
    Текстовые маркеры:
    ...
    Сейчас самое время обратить внимание на семантико-ассоциативную модель названия статьи о В. С. Соловьеве: «Немой пророк»– этооксюморонное сочетание, содержащее структурный код для представления образа В.С. Соловьева Д. С. Мережковским:
    
       Пророк ([width=1]) = обладатель профетического знания, модус 
       Немой ([width=1]) = неспособность к артикуляции, к выражению 
    ...
    То есть В. С. Соловьев одновременно:
       ...
       Является пророком (обладает истиной)
       Немой (не способен выразить эту истину)
       Если бы выразил пророчество, оно бы перестало быть пророчеством (превратилось бы в философский дискурс, потеряло бы аутентичность)
    ...
    Это означает, что когда В. С. Соловьев говорит (), он выражает нечто, но это нечто не является истиной (маска, неаутентичность); а когда В. С. Соловьев молчит (пророческое молчание), он хранит истину, но эта истина остается невыраженной
    ...
    В ПМО-терминах:
    Трагедия = 
        $$X_истинное : X_истинное $$ Z_публ     (истинное невыразимо)
        $$X_выраженное : ¬(истинное)           (выраженное неистинно)
        X_истинное $$ X_выраженное = $$         (расщепление)
    ...
    Ассоциативно-семантический и контекстуальный анализ функционирования концепта «омут» в этом сборнике показывает необходимость коррекции его интерпретации как эсхатологического закона неизбежного синтеза: омут как (B) неизбежный синтез,  
    Текстовые маркеры:
    Мережковский говорит о слиянии в , а не актуального осуществления: «Рано или поздно эти два противоположные течения встретятся и сольются в одном бездонном омуте» [Мережковский, «Мистические хулиганы»].
    ...
    Глаголы встретятсясливаютсяпредстоит маркируют будущность и неизбежность синтеза, но не его наличность в настоящем.
    Такое понимание позволяет нам формализовать предельный переход:
    Пусть для момента времени [width=1]определены две моды модуса [width=1]: 
    
    [width=1]

    где [width=1]– спецификатор, зависящий от времени. 
    Текущее состояние (конечное время): В любой момент конечного времени [width=1]две моды :
    
    [width=1]

    Они существуют параллельно, но не сливаются.
    Предельный переход (эсхатон): Введем оператор слияния [width=1](от греч. ὅμοιος – один, единство):
    
    [width=1]

    Таким образом, по мере приближения к концу истории (к эсхатону) две противоположные моды асимптотически приближаются друг к другу и в пределе достигают абсолютного единства [width=1]– полного синтеза созерцания и деяния. 
    Однако этот синтез остается недостижимым в исторической реальности. В. С. Соловьев умирает :
    
    [width=1]

    То есть В. С. Соловьев, по Д. С. Мережковскому, существует в промежутке [width=1], где синтез еще не произошел. Он исчерпывает противоречия, но не достигает их разрешения и как раз розовый башмачок в свете эсхатологии – это материальный знак недостижимости Он фиксирует привязанность к уже прошедшему времени в момент, когда требуется прыжок в будущее. Паника при его потере отражает онтологическое замешательство: утрата того малого материального знака, который связывал модус с миром конечного времени.
 ...
 & Соловьев-гностик 
       ...
       & 
    [width=1]
 & Гнозис, рационализм, консервативность, реставраторство 


      
       & 
    [width=1]
 & Три темпоральные условия образования мод 


      
       & 
    [width=1]
 & Революционный прагматизм структурно невозможен 
 ...
 & Публичный философ 
       ...
       & 
    [width=1]
 & Расщепление истины и выражаемости 


      
       & Розовый башмачок [width=1] & Амбивалентная модель прошлого 


      
       & 
    [width=1]
 & Неизбежный, но недостижимый синтез 
    ...
    Верификация против лингвориторического анализа служит ответом на этот вопрос. Она позволяет убедиться в том, что формализация не есть искажение, что ПМО-синтаксис сохраняет истину о тексте Мережковского и о образе В. С. Соловьева, выявленную лингвориторическим методом.
       ...
       Бинарная оппозиция «философ-созерцатель» / «человек-деятель» & Базовый модус [width=1](гностик) и несобственная модель [width=1](деятель)  &  


      
       Башмачок – материальный знак идеального чувства + привязка к прошлому & Башмачок как материализованная модель [width=1]и амбивалентная точка переходности  &  


      
       Омут – паремиологический образ слияния противоположностей & Омут как эсхатологический предельный переход [width=1] &  


      
       Расщепление на [width=1]и [width=1] & Два типа мод в разных моделях [width=1]и [width=1] &  
    ...
    После этого весьма интересного опыта применения ПМО для формализации структуры образа объекта литературно-критического текста остались открытыми некоторые теоретические вопросы. Так, например, для концепта слияния мы ввели оператор слияния Ω, который должен выражать эсхатологическое объединение противоположных мод и критически важен для нашего анализа. Однако в синтаксисе ПМО этот оператор не определен –он может быть рассмотрен как модусная сумма или операция на модах или на модусах – в любом случае без явного определения Ω наша формализация эсхатологического синтеза остается на уровне метафоры, облаченной в математические символы – это не формальный результат, а интуитивное выражение интуиции Д. С. Мережковского, и мы отдаем себе в этом отчет. 
    ...
    Это – метод не психологический и не исторический, но онтологический, и в этом смысле Д. С. Мережковский опережает современное ему литературоведение, предчувствуя те философские подходы к анализу личности и смысла, которые будут развиты только в XX веке, его метод предвосхищает позднейшие герменевтические и экзистенциальные подходы к анализу личности в литературе и становится прообразом критических практик, где язык и формальная структура рассматриваются не как украшение, но как основные генераторы смысла и онтологической истины.
       ...
       & 
    [width=1]
 & Показывает, что противоречие не логическое, но : структура модуса несовместима со структурой модели 


      
       & Башмачок как [width=1]и как точка переходности  & Дает точную формулировку того, как  служит медиумом идеального и одновременно препятствует переходу к действию 


      
       & Три собственные модели & Систематизирует интуицию о том, что Соловьев раскрывается полностью только через три временных измерения, ни одно из которых не достаточно 


      
       & 
    [width=1]
 & Формулирует парадокс: истина пророка именно в ее невыразимости; выражение истины ее уничтожает 


      
       & 
    [width=1]
 & Показывает, что синтез не актуален в истории, но структурно необходим как предел; это не проекция, но объективный закон бытия 
    ...
    Блок, А.А. (1906). Рыцарь-монах. 

М.: Путь. С. 103–115.
    Луговская, Е.Г., Грудина, Е.К. (2024). Экзистенциальное портретирование как критический метод: лингвориторический анализ статьи Д.С. Мережковского «Немой пророк». 
    Мережковский, Д.С. (1908/1991). Немой пророк. 

М.: Советский писатель. С. 128–135.
    Моисеев, В.И. (2002). Логика всеединства. М.: ПЕР СЭ.
    Моисеев, В.И. (2002). К аксиоматике Модальной Онтологии. 

Ростов н/Д: СКНЦ ВШ. С. 283–284.
    Моисеев, В.И. (2002). Projectively Modal Ontology. 

, 9. }
    Розанов, В.В. (1900). Еще о Вл. Соловьеве. , 3–4, 87–93.
    Трубецкой, Е.Н. (1913).  Т. 1–2. М.: Путь.
